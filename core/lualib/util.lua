...
function util.combine_icons(icons1, icons2, inputs, default_icon_size)
  ...
  for _,icon_to_add in pairs(icons2) do
    ...
    icon.scale = scale * (icon_to_add.scale or (0.5 * defines.constant.default_icon_size / icon.icon_size))
...
function util.product_amount(product)
  -- ref ItemProductPrototype::getExpectedNormalAmount
...
util.contains_value = function(list, value)
  for k, v in pairs(list) do
    if v == value then
      return true
    end
